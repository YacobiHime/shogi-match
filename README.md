...
- PC・タブレット・スマートフォンの縦横に合わせて盤の大きさ、文字サイズ、配置を自動で切り替える対局UI（PCでは棋譜欄を表示し、狭い画面では投了・盤面反転・助言設定をメニューへまとめる）
- 対局中・棋譜検討中に「ひふみんアイ」で盤面を上下反転
...
局面ごとに表示されたやこび姫の助言を復元します。タイトル画面を使う単体アクセスでも、
保存された対局があればタイトルを経由せず対局画面から再開します。棋譜解析では同じ局面を表示した瞬間に、
...
完成形の手前で止めてもよい「ほぼ完成形」(例: 本美濃に対する片美濃)も表示名・ID・必要な駒で登録でき、対局中はそこで続行か終了かを選ばせます。
...
「この局面から分岐」で応手別の変化を作成できます。参照したページがある場合は名称、HTTPS URL、
確認日を任意で登録できます。参照元がなくても定跡の保存とJSON書き出しが可能です。
