...
既存データは戦法・囲いの分類ごとにまとめて表示されます。戦法のメニュー分類・飛車分類と、囲いの系統・表示グループ・対応戦型も編集してJSONへ保存でき、分類名は候補からの選択だけでなく手入力にも対応しています。
案内手を直接編集し、「次の案内手を入力」で確認手順へ取り込みながら、先手と後手の手を盤上で交互に入力します。
一覧では「▲２六歩」のような日本語表記を確認でき、手をクリックすると指す直前の局面と矢印を盤上へ表示します。
相手の指し手は定跡を完全一致で固定するものではなく、その応手でも案内手が合法かを確認するための記録です。
戦法では「完成後に次の戦法を選ばせる」を有効にすると、選択時の案内文と派生戦法の追加・削除もJSONへ保存できます。
各案内手には「相手の8五に歩」「相手の8二に飛車」のような複数の盤上条件をAND指定できます。
...
保存した下書きは定跡ごとに削除でき、内蔵定跡は削除後に元の定義へ戻ります。
