...
コミットメッセージ等は日本語で書きます。件名は先頭に種別の接頭辞(`feat:`、`fix:`、`style:`、`perf:`、`refactor:`、`tune:`、`docs:`、`chore:`など)を付け、「fix: 角換わり手順の中断判定を修正。」のように、何を変えたかを体言止めまたは「〜する」形の一行で句点まで書きます。
また、"<fixやfeat等>: <内容>"という形で作成して。例："fix: 角換わり手順の中断判定を修正"
...
- 角交換は着手者だけでなく、盤上の角・馬と持ち駒の局面から判定します。相手から交換された場合は取り返しを案内し、交換成立後の手順へ合流させます。
- 定跡手は着手履歴の出現回数と、現在局面で同種の駒が目的地に到達したかを併用して消化します。予定の移動元が空でも別の同種駒が合法に目的地へ動ける場合はその手を案内し、到達も代替も不可能な場合に中断します。
- 対局準備のCPU角道設定では、開けたまま、開けてから閉じる、CPUから角交換、プレイヤーからの角交換待ち、閉じたままを選べます。具体的な角道指定は初手指定の次に優先し、定跡終了後のAI候補にも制約を維持します。交換待ち・閉じる設定などでは、CPUの角から相手の角を取りに行く手を除外します。
- WASM将棋エンジンはメインスレッドで動くため、CPU着手直後に重い助言探索を重ねないでください。先に着手を描画し、定跡安全確認・完成後候補は短時間探索で実行します。通常助言はCPU探索結果を再利用し、直後の重複再探索を行いません。
...
型チェックは導入時点で既存の型エラー175件が残っています。新たな型エラーを増やさず、段階的に修正してください。
